...
class RegisterUser(DataMixin, CreateView):
    ...
    # profile = User.objects.get(username=form_class)
    # print(profile, form_class)
    template_name = 'messenger/registrationPage.html'
    success_url = reverse_lazy('loginPage')

    # def create_profile(self, user):
    #     profile = self.create(user=user, id_user=user.id, number=user.number)
    #     profile.save()
    #     return profile
    ...
    #     person = Profile()
    #     person.name = user.objects.get("user")
    #     person.age = user.objects.get("id_user")
    #     person.save()
